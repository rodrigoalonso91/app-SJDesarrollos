import transformBlockSidesToLots from "../domain/TransformBlockSidesToLots"
...
describe("TransformBlockSidesToLots should", () => {
	...
	function transform(params: { externals: Array<Line>, internals: Array<Line> }) {
		results = transformBlockSidesToLots(params)
	}

	function expected(...expected: Array<Array<Coordinate>>) {
		const missing = expected.filter(x => !results.some(y => shapesAreEqual(x, y)))
		const exceeding = results.filter(x => !expected.some(y => shapesAreEqual(x, y)))
		if ([...missing, ...exceeding].length)
			throw Error(`Results: \n${results.map(result => JSON.stringify(result)).join("\n")}`)
	}
})

const shapesAreEqual = (shapeB: Array<Coordinate>, shapeA: Array<Coordinate>) => {
	...
	return (
		shapeA.every((a, i) => coordinatesAreEqual(a, shapeB[(offset + i) % total])) ||
		shapeA.every((a, i) => coordinatesAreEqual(a, shapeB[(total + offset - i) % total]))
	)
}
