...
describe("SplitSegmentOnIntersections should", () => {
	...
	it("return split segment if a parallel contains one end and shares the other", () => {
		...
		const PART_1: Segment = [
			{ x: 1, y: 1 },
			{ x: 3, y: 1 }
		]
		const PART_2: Segment = [
			{ x: 3, y: 1 },
			{ x: 4, y: 1 }
		]
	...
	it("return split segment to its minimum parts when two parallel segments overlap with it", () => {
		const SPLITTER_1: Segment = [
			{ x: 1, y: 1 },
			{ x: 3, y: 1 }
		]
		const SPLITTER_2: Segment = [
			{ x: 2, y: 1 },
			{ x: 4, y: 1 }
		]
	...
	function expectResultsToBe(expected: Array<Segment>) {
		const missing = expected.filter(
			(x) => !results.some((y) => segmentsAreRoughlyEqual(x, y))
		)
		const exceeding = results.filter(
			(x) => !expected.some((y) => segmentsAreRoughlyEqual(x, y))
		)
